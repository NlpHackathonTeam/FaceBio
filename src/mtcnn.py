...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_all=False):
        ...
        # Extract faces
        faces = self.extract(img, batch_boxes, save_path, batch_probs)
    ...
    def extract(self, img, batch_boxes, save_path, probs):
        ...
        faces = []
        for im, box_im, path_im, prob in zip(img, batch_boxes, save_path, probs):
            if box_im is None:
                faces.append(None)
                continue
            ...
            if self.keep_all and len(faces_im) > 0:
